...
>$x \sim N(0, \Sigma_{p\times p})$ and denote by $X = (x_1 \ x_2  \dots  x_n) \in \mathbb{R}^{p \times n}$ the data matrix. (Obverse $n$ times for $x_{p\times 1}$ 观察 $n$ 次 $p$ 维参数). $$\hat{\Sigma} := \frac{1}{n}XX^T = \frac{1}{n}\sum_{k=1}^n x_k x_k^T,$$ is an estimator for $\Sigma$.
...
$$\mathrm{P} \{ |\|X\| - E[\|X\|]| \geq \alpha \} \leq 2 \exp \left( -\frac{\alpha^2}{2} \right).$$ ^bjj1x5
...
>$$\psi_{\text{MP}}(x) = \underbrace{\frac{1}{2\pi \sigma^2 \gamma}}_{\substack{\text{Normalization}\\ \text{Coefficient}}} \cdot \frac{\sqrt{(\lambda_+ - x)(x - \lambda_-)}}{x}$$where $\lambda_+ = \sigma^2 (1 + \sqrt{\gamma})^2$ &  $\lambda_- = \sigma^2 (1 - \sqrt{\gamma})^2$.
...
>$$\frac{1}{p} \sum_{i=1}^{p} \frac{1}{\lambda_i - z} = \operatorname{ntr} \left[ (A - zI_p)^{-1} \right],$$where $\operatorname{ntr} = \frac{1}{p} \operatorname{Tr}$ is the normalized trace on $\mathbb{R}^{p \times p}$.

By [[Char2 Wigner Ensemble & SemiCircle Law#^v7zq7x|The eigenvalues of Resolvent]]

>[!lemma] Lemma. 4.6 很重要的一个技巧
>Let $x \sim N(0, I_{p})$ be a standard Gaussian vector in $\mathbb{R}^{p}$ and $B \in \mathbb{R}^{p \times p}$ a deterministic or random matrix independent from $x$. Then $$E_{x}[x^{T} B x]=\operatorname{Tr}(B).$$
